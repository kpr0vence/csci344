...
class AccessTokenEndpoint(Resource):
    ...
    def post(self):
        ...
        # 1. Query Database for a user matching the username
        ...
        # 2. Check that the password matches
            # If passwords don't match, throw an error and terminate early
        ...
        # 3. We know user has good login credentials. Give them an access/refresh token
...
class RefreshTokenEndpoint(Resource):
    ...
    def post(self):
        ...
        if exp_timestamp > now:
            ...
            refresh_token_new = flask_jwt_extended.create_refresh_token(identity=identity)
            return Response(
                json.dumps(
                    {
                        "access_token": access_token,
