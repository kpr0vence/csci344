...
def get_all_posts():
    ...
    # query = Post.query    #shows the actual query that is generated
    # print(query)
    posts = Post.query.filter_by(user_id=30).all()
    for post in posts:
        print(post.caption)
    return posts
...
# TASK 2
@with_app_context
def get_n_posts(count=10):  #Has a default parameter of 10, if given a value it will replace 10 with that value
    if (count > 50):    #A reasonable check to make sure that some rando can't ask for like 7 billion posts (which would suuuuuck)
        count = 50          #This would be a DDOS (denial of service) attack, because no one else would be able to use the system while
                            #This super expensive request is taking up all the bandwith
    #Also consider protecting against words (instead of numbers) being passed in
        #Or worse pass in a bit of code ex. count='DROP DATABASE orm_test;'     (SQL Injection Attack)
...
# SELF FUNCTION
@with_app_context
def add_new_user(username, firstname, lastname, myEmail):
    # Create a new python user object
    new_user = User(
    ...
    new_user.password_plaintext = 'secure'
    new_user.set_password(new_user.password_plaintext)
    db.session.add(new_user)    #Creates the new user in the db
    db.session.commit()              #comits it so that it saves
    db.session.refresh(new_user)        #refreshes the user
    print(new_user)
    return new_user
...
@with_app_context
def tester():
    add_new_user('algae', 'Al', 'Gallager', 'alG@gmail.com')
    # print("\n----------------------")
    ...
    # print("---------------------------------")
    # posts = get_posts_liked_by_user(19)
    # print(posts)
