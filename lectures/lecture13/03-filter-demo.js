...
const startsWithL = (name) => name[0].toLowerCase() === "l";

const losers = names.filter(startsWithL);
console.log(losers + " took the L.");

// losers.forEach(loser => console.log("Hello " + loser));
const greetingsLosers = losers.map(loser => `Hello ${loser}` );
console.log(greetingsLosers);

const losers2 = names.filter(startsWithL).map(loser => `Hello ${loser}`);   //Doing it all in one line
console.log(losers2);
