...
function insertTracks(myTracks) {
    //First remove all left over tracks

    let target = document.querySelector("#track-list");

    target.innerHTML = "";

    for (let track of myTracks) {
        let snippet = generateSnippet(track);

        target.insertAdjacentHTML("beforeend", snippet);
    }
 }

function generateSnippet(track) {
...
const selectTracks = (ev) => {
    ...
    //Show all on blank search
    if (searchTerm === "") {
        insertTracks(tracks);
        return;
    }

    //Filter ctracks to just be that 
    let myTracks = tracks.filter( (track) => {
        //do album, artist, and title
        let songName = track.name.toLowerCase();
        let artistName = track.artist.name.toLowerCase();
        let albumName = track.album.name.toLowerCase();

        if (songName.includes(searchTerm.toLowerCase())) {
            return true;
        }
        if (artistName.includes(searchTerm.toLowerCase())) {
            return true;
        }
        if (albumName.includes(searchTerm.toLowerCase())) {
            return true;
        }
        return false;
    });
    ...
    //Call 
    console.log("Tracks that passed the filter: " +myTracks);
    insertTracks(myTracks);
};