...
function getImgUrls() {
    let postsWithImages = statuses.filter(post => (post.image_url))
    console.log(postsWithImages);
    let imgUrls = postsWithImages.map(post => post.image_url);
    console.log(imgUrls);
...
function getScreenNames() {
    let famousPeople = statuses.filter(post => post.retweet_count >= 500);
    let namesOfFamousPeople = famousPeople.map(post => post.screen_name);
    console.log(namesOfFamousPeople);

    return namesOfFamousPeople;
}

function pickLarger(one, two) {
    if (one.retweet_count > two.retweet_count) {
        return one;
    } else {
        return two;
    }
}
function getMostFamousPost() {
    let famousPost = statuses.reduce((currentLargest, newComer) => pickLarger(currentLargest, newComer));
    console.log("Most famous post is by " +famousPost.screen_name+ " and has " +famousPost.retweet_count+ " retweets");
    return famousPost;
}

function getAllRetweets() {
    let allRTs = statuses.reduce( (accumulator, tweet) => (accumulator + tweet.retweet_count), 0);
    console.log(`The total number of retweets is ${allRTs}.`);
    return allRTs;
}